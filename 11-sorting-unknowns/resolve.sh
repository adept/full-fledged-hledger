...
while true ; do
    ...
    regexp=$(sk --header="Fine-tune regexp. Searching in ${dir}/*.rules and ${dir}/csv" \
                ...
                -c "rg -i --color=always --line-number {q} $(ls ${dir}/*.rules | paste -s -d' ') ${dir}/rules.psv ${dir}/csv" | head -n1)
    ...
    comment=$((sk --header="Enter comment (prepend : to inhibit selection)" --print-cmd --ansi -i \
                  -c "rg --color=always --line-number {q} /tmp/comments.txt" || true) | tail -n1 | sed -e 's/^[0-9]*://')
